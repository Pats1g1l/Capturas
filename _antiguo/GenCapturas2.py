...
if not os.path.exists(vCarpetaDestino):
    logging.info("Creando subcarpeta: " + vCarpetaDestino)
    os.mkdir(vCarpetaDestino)


# Encuentra la ventana del DSS por su título
try:
    window = gw.getWindowsWithTitle('DSS Client')[0]  # Reemplaza 'Notepad' con el título de la ventana que deseas activar
...
try:
    ...
    if conexion.is_connected():
        ...
        print("Armando consulta a la BD: " + consulta_SQL)
        logging.info("Armando consulta a la BD: " + consulta_SQL)

        # Ejecutar la consulta
        print("Ejecutando consulta a la BD")
        logging.info("Ejecutando consulta a la BD")
        ...
        for (nombre_empresa, EstadoCamNombre, rut_empresa, email_empresa, seguridad_correo, empresa_guardia) in cursor:
            print(f"--Nombre: {nombre_empresa}, EstadoCamNombre: {EstadoCamNombre}, RUT: {rut_empresa}, Email: {email_empresa}, Seguridad: {seguridad_correo}, Guardia: {empresa_guardia}")
            logging.info(f"--Nombre: {nombre_empresa}, EstadoCamNombre: {EstadoCamNombre}, RUT: {rut_empresa}, Email: {email_empresa}, Seguridad: {seguridad_correo}, Guardia: {empresa_guardia}")
        ...
        for (nombre_empresa, EstadoCamNombre, rut_empresa, email_empresa, seguridad_correo, empresa_guardia) in cursor:
            ...
            print("Preparando pantallazo [" + EstadoCamNombre  + "] para la empresa: [" + str(nombre_empresa)+"]")
            logging.info("Preparando pantallazo [" + EstadoCamNombre  + "] para la empresa: [" + str(nombre_empresa)+"]")
            ...
            #PRINT DE PANTALLA
            nombre_archivo = f"{EstadoCamNombre}_{fecha_hora_actual}.jpg"
